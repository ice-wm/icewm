...
=item B<QuickSwitchPersistence>=0

Time in seconds to remember the state of Alt+Tab.

=item B<QuickSwitchRaiseCandidate>=0
...
=item B<WinMenuItems>="rmsnxfhualytiecw"
...
They can also be set in the F<preferences> file, but themes will override
the values set there.  To override the theme values, the settings should
be set in F<prefoverrides> file: see L<icewm-prefoverrides(5)>.  Default
values are shown following the equal sign.

=head3 THEME DESCRIPTION
...
=head3 THEME BORDERS, ICONS, MARGINS AND BUTTONS
...
=item B<MenuButtonIconVertOffset>=0  [-128-128]
...
=item B<ColorKeyboardLayoutText> = ""
...
=item B<CycleBackgroundsPeriod>=0
